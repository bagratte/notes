...
`Canvas/utils.ts` exports `strokesToSvgString` (strokes → SVG markup at a given pixel size, reusing the same `getStroke`/`svgPathFromStroke` rendering as everywhere else, with an optional transparent background for compositing), `rasterizeSvg` (SVG string → `HTMLCanvasElement`), `renderStrokesCrop` (strokes → canvas cropped to their bounding box + 12 px padding, white background, 2×), and `copyImageToClipboard` (the single OS-clipboard write path). This writes a real PNG to the **OS** clipboard — distinct from the in-app stroke clipboard used by stroke-select copy/paste (above), which round-trips between sections in the same note session. The OS clipboard is restricted to blob types (`image/png`, `text/plain`, `text/html`); raw `image/svg+xml` isn't reliably pasteable as an image outside vector tools, hence rasterizing.

`copyImageToClipboard(render, { copyId? })` takes a **render callback**, not a finished canvas: it reaches `navigator.clipboard.write()` synchronously and hands the async rendering to the `ClipboardItem` promise. Safari only honours a clipboard write issued while user activation is live, so awaiting the render before calling `write()` breaks on iPad (Chrome's 5 s transient-activation window forgives it, which is why the older shape appeared to work). It rejects when `navigator.clipboard` is missing — the API is gated on a secure context, so plain-HTTP LAN origins have none; callers with other work to do must catch.

`copyId` (generate via `newCopyId()` — deliberately not `crypto.randomUUID()`, which is itself secure-context-only) adds a `text/html` flavour carrying `CLIPBOARD_COPY_ID_ATTR` (`data-notes-copy="<id>"`) alongside the PNG. It is a marker for a **future** OS-clipboard paste handler: matching it against `NoteEditor`'s `clipboard.id` distinguishes "this is the copy the user just made here, paste it as vector strokes" from "this came from another app, paste it as an image" — the ordering problem that recency alone can't settle. Nothing reads it yet. Only pass `copyId` from a copy that also populates the in-app stroke clipboard; the whole-section and region image copies must not claim to be stroke copies.
...
A `Region` can be linked to more than one note (many-to-many via the backend's `region_sections` join table). The `Region` type carries `sections: { id: number; note: { id: number; name: string } }[]` directly from `GET /regions/` — there is no frontend enrichment step or per-section fetch/cache; the backend response already embeds everything needed.

Creating a brand-new region (drawing a rectangle in `select-region` mode, "Create Note" / "Add to Note" on the pending-selection popup) still calls `sectionsApi.create` then `regionsApi.create` — one call that creates the region and its first link atomically (`handleRegionComplete` / `handleRegionAddToNote` in `useDocumentViewer.ts`). Linking an *existing* region to another note goes through `sectionsApi.create` then `regionsApi.linkSection(regionId, sectionId)` — `POST /regions/{id}/sections` — which attaches a new section to an already-placed region without creating a second region row (`handleRegionLinkNewNote` / `handleRegionLinkExistingNote`). Navigating to the note after linking is intentional — the new section needs to be visible immediately.

### Region context menu

Left-clicking a region opens its note directly when it has exactly one linked note. When it has more than one, left-click instead opens the same fixed-position context menu as long-press/right-click, with **Open Note** pre-expanded into its submenu (`openNoteExpanded` state) so the user picks which note to open rather than guessing.

Long-pressing on touch or right-clicking opens the context menu with: **Open Note** (a submenu listing every linked note when there's more than one, otherwise a single direct action — submenu entries get the same hover/tap `NoteStrokePreview` treatment described below), **Create Note**, **Add to Note ›** (reusing the same notes-fetch/hover-preview UI as the pending-selection popup, via the shared `renderNoteLinkItems` helper — already-linked notes are excluded from the "Add to Note" list), **Copy**, and **Resize Region**. There is no "Delete" item on this menu — removing a region's link to a note is done from inside the note itself (deleting the region-backed section via the existing section-delete control). The backend (`cleanup_orphaned` in `app/services/regions.py`) then deletes the region too if that was its last remaining section link; the note itself is left as-is (even if now empty). Resize handles only appear after the user explicitly enters edit mode via this menu — there are no hover-activated handles. `suppressRegionClickRef` prevents a click event from firing after a long-press that opened the menu.
...
**Zoom**: `fitMode` is `"width" | "page" | "manual"`; page zoom beyond "fit" grows the page and scrolls (no CSS clamping/distortion — `.pageWrap` has no `max-width`, and `.pageStack` left-aligns overflowing pages via `margin: auto` centering that collapses once the page is wider than the viewport). The toolbar's dedicated "Fit to visible content width" button (`fitToContentWidth`) is a one-shot action, not a `fitMode` value: it samples the current page's rendered canvas via `computeContentBounds` to detect the horizontal extent of non-background content (trimming blank margins), then sets `fitMode: "manual"` with a computed `manualScale` and scrolls horizontally to the content — so it behaves like any other manual zoom (holds steady across page navigation, steps via `zoomIn`/`zoomOut`) and only recomputes on the next click. The zoom percentage next to `-`/`+` is a directly editable text input (`zoomInput`/`handleZoomInputSubmit`, same commit-on-Enter/blur pattern as the page-number input) for typing an arbitrary value (e.g. `456`), clamped to the `ZOOM_STEPS` min/max and applied as `fitMode: "manual"`; the Fit Width/Fit Page/Actual Size popover now opens from a separate caret button beside it. The "Lock horizontal scrolling" toggle (`horizontalScrollLocked`) next to the content-fit button sets `overflow-x: hidden` on the scroll container (blocking wheel/trackpad/scrollbar horizontal movement) and makes the hand-tool pan handler ignore horizontal drag delta — vertical scrolling and page navigation are unaffected, and programmatic horizontal positioning (e.g. from `fitToContentWidth`) still applies.
