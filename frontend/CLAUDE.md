...
- **Mouse, outside touch mode** uses native HTML5 drag-and-drop. Rows are only `draggable` when **not** in touch mode (`draggable={!isTouch}`) — Chromium's native *touch* drag-and-drop fires on a long-pressed `draggable` element and would otherwise fight the gesture below.
- **Touch, pen, and (in touch mode) mouse** use `useTouchReorder` (`src/components/Sidebar/useTouchReorder.ts`), a pointer-event gesture: drag over a sibling to pick a before/after position via `elementFromPoint` (rows carry `data-drag-type` / `data-drag-id`), release to commit. Pickup differs by input — touch/pen need a 350 ms still hold (moving more than 10 px first is treated as a scroll and cancels); the mouse, which has no scroll ambiguity, picks up on a 5 px drag with no hold (so a plain click still navigates). In touch mode the mouse is routed here (via `isTouchRef`) because native DnD is off there. A non-passive `touchmove` `preventDefault` suppresses list scrolling only while a row is held. The post-drag click is swallowed via `suppressClickRef`, which self-clears after 400 ms so it can't eat a later tap (touch/pen drags synthesize no click to consume it).
...
`commitReorder` reads the latest tree from a `dataRef` (not closure state) because the gesture commits from `window` pointer listeners. The mouse falls through to native DnD only when not in touch mode; in touch mode `useTouchReorder` handles it.
